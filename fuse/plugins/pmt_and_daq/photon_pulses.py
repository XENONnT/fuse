...
class PulseWindow(FuseBasePlugin):
    ...
    def setup(self):
        ...
        self.pulse_left_extenstion = (
            np.int64(2 * self.samples_to_store_before)
    ...
    def compute(self, propagated_photons, start, end):
        ...
        single_photon_pulses = np.zeros(
            len(propagated_photons), dtype=strax.interval_dtype
        )
...
def concat_overlapping_hits(hits, extensions, pmt_channels, start, end):
    ...
    last_hit_in_channel = np.zeros(
        ...
        dtype=(
            ...
            + [
                (
                    ("End time of the interval (ns since unix epoch)", "endtime"),
                    np.int64,
                ),
                (("pulse_id"), np.int64),
            ]
    ...
    for i, hit in enumerate(hits):
        ...
        if found_no_hit_for_channel_yet:
            ...
            if hits_overlap_in_channel:
                ...
                hitlet_length = (
                    last_hit_in_channel["endtime"] - last_hit_in_channel["time"]
                )
