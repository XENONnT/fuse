        ...
        for k in self.uniform_to_pmt_ap.keys():
            for q in self.uniform_to_pmt_ap[k].keys():
                if isinstance(self.uniform_to_pmt_ap[k][q], list):
                    self.uniform_to_pmt_ap[k][q] = np.array(
                        self.uniform_to_pmt_ap[k][q]
                    )
        ...
        for element in element_list:
            ...
            if "Uniform" in element:
                ...
                if len(amplitude_cdf.shape) == 2:
                    ap_amplitude = (
                        np.argmin(
                            np.abs(amplitude_cdf[sel_photon_channel] - rU1[:, None]),
                            axis=-1,
                        )
                        * amplitude_bin_size
                    )
                    ...
                    ap_amplitude = (
                        np.argmin(
                            np.abs(amplitude_cdf[None, :] - rU1[:, None]), axis=-1
                        )
                        * amplitude_bin_size
                    )
