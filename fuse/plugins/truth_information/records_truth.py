...
class RecordsTruth(strax.Plugin):
    ...
    dtype = [#(("Clusters contributing to the record", "contributing_clusters"), np.int16, self.max_contributing_channels_in_truth),
    ...
    def compute(self, raw_records, propagated_photons):
        ...
        for channel in np.unique(raw_records["channel"]):
            ...
            records_in_channel = records_per_channel[np.argwhere(unique_records_channels == channel)[0][0]]
            photons_in_channel = photons_per_channel[np.argwhere(unique_photon_channels == channel)[0][0]]
...
def split_photons_by_channel(propagated_photons):
    ...
    unique_photon_channels, split_position = np.unique(propagated_photons_sorted["channel"], return_index=True)
