...
class ClusterTagging(strax.Plugin):
    ...
    def compute(self, peaks, interactions_in_roi, events, propagated_photons):
        ...
        for i, (peaks_of_event, event_i, photon_of_event) in enumerate(zip(peaks_in_event, events, photon_in_event)):

            peak_photons = strax.split_touching_windows(photon_of_event, peaks_of_event)

            peak_name_dict = {"main_s1": "s1_index",
                              "main_s2": "s2_index",
                              "alt_s1": "alt_s1_index",
                              "alt_s2": "alt_s2_index",
                            }
            ...
            for peak_name in peak_name_dict.keys():
                ...
                if peak_idx >= 0:
                    ...
                    contributing_clusters_of_peak, photons_in_peak = np.unique(photons_of_peak['cluster_id'], return_counts=True)
                    photons_in_peak = photons_in_peak[contributing_clusters_of_peak>0]
                    contributing_clusters_of_peak = contributing_clusters_of_peak[contributing_clusters_of_peak>0]

                    matching_clusters = np.argwhere(np.isin(interactions_in_roi["cluster_id"], contributing_clusters_of_peak))
