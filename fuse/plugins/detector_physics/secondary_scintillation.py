...
class SecondaryScintillation(FuseBasePlugin):
    ...
    data_kind = {
        "s2_photons": "individual_electrons",
        "s2_photons_sum": "interactions_in_roi",
    }

    dtype_photons = [
        (
            ("Number of photons produced by the extracted electron", "n_s2_photons"),
            np.int32,
        ),
    ] + strax.time_fields
    dtype_sum_photons = [
    ...
    save_when = immutabledict(
        s2_photons=strax.SaveWhen.TARGET, s2_photons_sum=strax.SaveWhen.ALWAYS
    )
    ...
    def compute(self, interactions_in_roi, individual_electrons):
        ...
        if len(interactions_in_roi[mask]) == 0:
            empty_result = np.zeros(
                len(interactions_in_roi), self.dtype["s2_photons_sum"]
            )
            empty_result["time"] = interactions_in_roi["time"]
            empty_result["endtime"] = interactions_in_roi["endtime"]

            return dict(
                s2_photons=np.zeros(0, self.dtype["s2_photons"]),
        ...
        electron_gains = np.repeat(
            sc_gain, interactions_in_roi[mask]["n_electron_extracted"]
        )
        ...
        sum_photons_per_interaction = np.add.reduceat(
            n_photons_per_ele, np.r_[0, electron_indices[:-1]]
        )

        n_photons_per_ele = np.clip(n_photons_per_ele, 0, None)

        reorder_electrons = np.argsort(
            individual_electrons, order=["order_index", "time"]
        )

        result_photons = np.zeros(
            len(n_photons_per_ele), dtype=self.dtype["s2_photons"]
        )
        ...
        result_sum_photons = np.zeros(
            len(interactions_in_roi), dtype=self.dtype["s2_photons_sum"]
        )
        ...
        return dict(s2_photons=result_photons, s2_photons_sum=result_sum_photons)
