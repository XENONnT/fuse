...
class ElectronDrift(FuseBasePlugin):
    ...
    dtype = [
        ...
        (
            ("Mean drift time of the electrons in the cluster [ns]", "drift_time_mean"),
            np.int32,
        ),
        (
            (
    ...
    def in_charge_sensitive_volume(self, xy_int, z_int):
        ...
        if self.enable_field_dependencies["survival_probability_map"]:
            p_surv = self.field_dependencies_map(
                z_int, xy_int, map_name="survival_probability_map"
            )
    ...
    def get_s2_drift_time_params(self, xy_int, z_int):
        ...
        drift_time_spread = np.sqrt(
            2 * diffusion_constant_longitudinal * drift_time_mean
        )
    ...
    def get_avg_drift_velocity(self, z, xy):
        ...
        if self.enable_field_dependencies["drift_speed_map"]:
            drift_v_LXe = self.field_dependencies_map(
                z, xy, map_name="drift_speed_map"
            )  # mm/µs
