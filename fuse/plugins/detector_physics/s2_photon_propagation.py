...
class S2PhotonPropagationBase(FuseBaseDownChunkingPlugin):
    ...
    def setup(self):
        ...
        self.pmt_mask = (
            np.array(self.gains) > 0
        )  # Converted from to pe (from cmt by default)
    ...
    def compute(self, individual_electrons, interactions_in_roi, start, end):
        ...
        electron_time_gaps = (
            individual_electrons["time"][1:] - individual_electrons["time"][:-1]
        )
        ...
        if n_chunks > 1:
            ...
            for electron_group in electron_chunks[:-1]:
                interactions_chunk = interactions_in_roi[mask][
                    np.unique(electron_group["order_index"])
                ]
                # interactions_chunk = interactions_in_roi[mask][np.min(index_group):np.max(index_group)+1]
                positions = np.array(
                    [interactions_chunk["x_obs"], interactions_chunk["y_obs"]]
                ).T
                ...
                _photon_timings += np.repeat(
                    electron_group["time"], electron_group["n_s2_photons"]
                )
        ...
        # And the last chunk
        interactions_chunk = interactions_in_roi[mask][
            np.unique(electron_chunks[-1]["order_index"])
        ]
        ...
        _photon_timings += np.repeat(
            electron_chunks[-1]["time"], electron_chunks[-1]["n_s2_photons"]
        )
    ...
    def photon_channels(self, n_electron, z_obs, positions, drift_time_mean, n_photons):
        ...
        pattern = np.divide(
            pattern, sum_pat, out=np.zeros_like(pattern), where=sum_pat != 0
        )
        ...
        for i, n_ph in enumerate(n_photons):
            ...
            if np.isnan(pat).sum() > 0:
                ...
                _photon_channels = self.rng.choice(
                    channels, size=n_ph, p=pat, replace=True
                )
    ...
    def s2_pattern_map_diffuse(self, n_electron, z, xy, drift_time_mean):
        ...
        xy_multi = (
            np.repeat(xy, n_electron, axis=0) + hdiff
        )  # One entry xy per electron
        ...
        for ix, ne in enumerate(n_electron):
            s = slice(n0, n0 + ne)
            pattern[ix, :] = np.average(
                self.s2_pattern_map(xy_multi[s][mask[s]]), axis=0
            )
            n0 += ne
    ...
    def singlet_triplet_delays(self, size, singlet_ratio):
        ...
        delay = self.rng.choice(
            [t1, t3], size, replace=True, p=[singlet_ratio, 1 - singlet_ratio]
        )
...
class S2PhotonPropagation(S2PhotonPropagationBase):
    ...
    def setup(self):
        super().setup()
        log.debug(
            f"Using Garfield GasGap luminescence timing and optical propagation with plugin version {self.__version__}"
        )

    def photon_timings(self, positions, n_photons, _photon_channels):

        _photon_timings = self.luminescence_timings_garfield_gasgap(
            positions, n_photons
        )
    ...
    def luminescence_timings_garfield_gasgap(self, xy, n_photons):
        ...
        d_gasgap = (
            self.s2_luminescence_map["gas_gap"][1]
            - self.s2_luminescence_map["gas_gap"][0]
        )

        cont_gas_gaps = self.garfield_gas_gap_map(xy)
        draw_index = np.digitize(cont_gas_gaps, self.s2_luminescence_map["gas_gap"]) - 1
        diff_nearest_gg = (
            cont_gas_gaps - self.s2_luminescence_map["gas_gap"][draw_index]
        )
...
def draw_excitation_times(
    inv_cdf_list, hist_indices, nph, diff_nearest_gg, d_gas_gap, rng
):
    """
...
def _luminescence_timings_simple(
    ...
    for i in range(n):
        ...
        emission_time[ci : ci + npho] = np.interp(probabilities, y / y[-1], t).astype(
            np.int64
        )
        ci += npho
...
def simulate_horizontal_shift(
    ...
    hdiff_radial = rng.normal(0, 1, np.sum(n_electron)) * np.repeat(
        hdiff_stdev_radial, n_electron
    )
    ...
    for i in range(len(matrix)):
        result[start_idx[i] : stop_idx[i]] = (
            np.ascontiguousarray(split_hdiff[i]) @ matrix[i]
        )
