...
class NestYields(FuseBasePlugin):
    ...
    def setup(self):
        ...
        if self.deterministic_seed:
            ...
            log.debug(
                f"Generating nest random numbers starting with seed {self.short_seed}"
            )
        else:
...
class BBF_quanta_generator:
    ...
    def get_ER_quanta(self, energy, field, par_dict):
        ...
        Nq = np.clip(
            np.round(self.rng.normal(Nq_mean, np.sqrt(Nq_mean * par_dict["fano"]))),
            0,
            np.inf,
        ).astype(np.int64)
    ...
    def get_NR_quanta(self, energy, field, par_dict):
        Nq_mean = energy / par_dict["W"]
        Nq = np.round(
            self.rng.normal(Nq_mean, np.sqrt(Nq_mean * par_dict["fano"]))
        ).astype(np.int64)
