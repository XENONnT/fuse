...
class ChunkInput(FuseBasePlugin):
    ...
    def compute(self):
        ...
        try:
            chunk_data, chunk_left, chunk_right, source_done = next(
...
class file_loader:
    ...
    def output_chunk(self):
        ...
        if self.event_rate > 0:
            ...
            if self.file.endswith(".root"):
                log.warning(
                    "Using event times from root file is not recommended! Use a source_rate > 0 instead."
                )
        ...
        log.info(
            f"Removing {np.sum(~delay_cut)} ( {np.sum(~delay_cut)/len(delay_cut) *100:.4} %) interactions later than {self.cut_delayed:.2e} ns."
        )
        ...
        if (len(chunk_start) > 1) & (len(chunk_end) > 1):
            ...
            self.chunk_bounds = np.append(
                chunk_start[0] - self.first_chunk_left, chunk_bounds
            )
        ...
        for c_ix, chunk_left, chunk_right in zip(
            ...
            yield inter_reshaped[
                chunk_idx == c_ix
            ], chunk_left, chunk_right, source_done
    ...
    def _load_root_file(self):
        ...
        if self.arg_debug:
            ...
            if self.kwargs["entry_start"] is not None:
                log.debug(
                    f'Starting to read from {cutby_string} {self.kwargs["entry_start"]}'
                )
            if self.kwargs["entry_stop"] is not None:
                log.debug(
                    f'Ending read in at {cutby_string} {self.kwargs["entry_stop"]}'
                )
        ...
        interactions["x_pri"] = ak.broadcast_arrays(
            xyz_pri["x_pri"], interactions["x"]
        )[0]
        interactions["y_pri"] = ak.broadcast_arrays(
            xyz_pri["y_pri"], interactions["x"]
        )[0]
        interactions["z_pri"] = ak.broadcast_arrays(
            xyz_pri["z_pri"], interactions["x"]
        )[0]
    ...
    def _awkwardify_df(df):
        ...
        dictionary = {
            ...
            "parenttype": reshape_awkward(
                np.array(df["parenttype"], dtype=str), evt_offsets
            ),
            "parentid": reshape_awkward(df["parentid"].values, evt_offsets),
            "creaproc": reshape_awkward(
                np.array(df["creaproc"], dtype=str), evt_offsets
            ),
