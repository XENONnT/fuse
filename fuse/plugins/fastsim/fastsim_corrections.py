...
class FastsimCorrections(FuseBasePlugin):
    ...
    dtype = [
                ...
                (('Correction added to alt_s2_r_naive for field distortion (cm)',
                  'alt_s2_r_field_distortion_correction'), np.float32),
                (('Main interaction angular position (radians)', 'theta'), np.float32),
                (('	Alternative S2 (rel. main S1) interaction angular position (radians)', 'alt_s2_theta'), np.float32)
            ] + strax.time_fields
    ...
    def compute(self, fastsim_events):
        ...
        for alt_s2, alt, fdc in [('', '', ''), ('alt_s2_', 'alt_', '_fdc')]:
            fastsim_events[f'{alt_s2}r_field_distortion_correction'] = self.fdc_map_fuse(
                np.array([fastsim_events[f'{alt}s2_x'],
                          fastsim_events[f'{alt}s2_y'],
                          fastsim_events[f'{alt_s2}z_naive']]).T)
            with np.errstate(invalid='ignore', divide='ignore'):
                r_cor = fastsim_events[f'{alt_s2}r_naive'] + fastsim_events[f'{alt_s2}r_field_distortion_correction']
                scale = np.divide(r_cor, fastsim_events[f'{alt_s2}r_naive'], out=np.zeros_like(r_cor),
            ...
            result[f'{alt_s2}theta'] = np.arctan2(fastsim_events[f'{alt_s2}y{fdc}'],
                                                  fastsim_events[f'{alt_s2}x{fdc}'])
        ...
        for alt_s2_interaction, alt in [('', ''), ('alt_s2_interaction_', 'alt_')]:
            lifetime_corr = np.exp(fastsim_events[f'{alt_s2_interaction}drift_time'] / self.electron_lifetime_liquid)

            # S2(x,y) corrections use the observed S2 positions
            xy = np.vstack([fastsim_events[f'{alt}s2_x'], fastsim_events[f'{alt}s2_y']]).T
            result[f'{alt}cs2'] = (fastsim_events[f'{alt}s2_area'] * lifetime_corr / self.s2_correction_map(xy))
